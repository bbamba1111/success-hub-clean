...
export function CoPilotChatModal({
  ...
  const sendWelcomeMessage = async (convId: string) => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/chat/co-pilot", {
        method: "POST",
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        console.error("[Co-Pilot] Non-JSON response:", text);
        throw new Error("Server returned non-JSON response");
      }

      const data = await response.json();
      console.log("[Co-Pilot] Response data:", data);
      ...
      if (data.message) {
        ...
        console.error("[Co-Pilot] API error:", data.error);
        setMessages((prev) => [
          ...prev,
